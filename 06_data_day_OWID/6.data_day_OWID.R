...
# Male plot
ggraph(graph, layout = "linear") + 
  geom_edge_arc(aes(edge_colour = group, edge_width = group)) +
  geom_node_text(aes(label = name), repel = FALSE, size = 6, nudge_y = -1,
                 color = rep(c("white", NA, NA, NA), length.out = 89)) +
  scale_edge_color_manual(values = c(NA, "grey25", "#39FF14")) +
  scale_edge_width_manual(values = c(1, 1, 3)) +
  labs(title = "Life Expectancy by Sex",
       subtitle = "In every country the life expectancy of <span style='color:#BB29BB'>**women**</span> is higher than the life expectancy of <span style='color:#39FF14'>**men**</span>") +
  theme_void(base_family = "Lato") +
  theme(plot.margin = margin(t = 40, r = 50, b = 0, l = 50),
...
ggsave("30chartchallenge_6_male_avg_2022.png", width = 16.5, height = 11.7, units = "in", dpi = 320)
...
ggsave("30chartchallenge_6_female_avg_2022.png", width = 16.5, height = 11.7, units = "in", dpi = 320)
...
ggsave("vc_male.png", width = 16.5, height = 11.7, units = "in", dpi = 320)
...
# Female plot
ggraph(graph, layout="linear") + 
  geom_edge_arc(aes(edge_color = group, edge_width = width, alpha = group), strength = -1) +
  geom_node_text(aes(label=name), repel = FALSE, nudge_y = .5,
                 color = rep(c("white", NA, NA, NA), length.out = 89)) +
  scale_edge_color_manual(values = c("black", "grey75", "#BB29BB")) +
  scale_edge_alpha_manual(values = c(0, .15, 1)) +
  labs(caption = "Visualization by Pablo Alvarez | Data from Our World In Data: Life Expectancy at birth for the year 2020.\nThe width of each line depicts the number of countries with a given life expectancy for that sex.") +
  theme_void(base_family = "Lato") +
  theme(plot.margin = margin(t = 0, r = 50, b = 0, l = 50),
        panel.background = element_rect(fill = "black", color = "black"),
        plot.background = element_rect(fill = "black", color = "black"),
...
ggsave("vc_female.png", width = 16.5, height = 11.7, units = "in", dpi = 320)
